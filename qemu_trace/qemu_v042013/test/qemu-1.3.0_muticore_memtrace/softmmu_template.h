	...
	if ((addr & TARGET_PAGE_MASK) == (tlb_addr & (TARGET_PAGE_MASK | TLB_INVALID_MASK))) {
		if (tlb_addr & ~TARGET_PAGE_MASK) {
			...
			if (memtrace_code){
				...
				varify_vaddr = env->tlb_table[mmu_idx][index].addr_code+(physaddr&0xfff);
				if(varify_vaddr == trace_pc)
					tb_paddr_cache = physaddr;
			}else if(trace_recoding){
				physaddr = ((env->iotlb[mmu_idx][index]) & TARGET_PAGE_MASK) + addr;
				trace_pc = env->eip + env->segs[R_CS].base;
				varify_vaddr = env->tlb_table[mmu_idx][index].addr_code;
				if(varify_vaddr!=(trace_pc & 0xfffffffffffff000)){
					raw_trace[0][raw_trace_idx] = 0x0;
					raw_trace[1][raw_trace_idx] = env->cpuid_apic_id;
					raw_trace[2][raw_trace_idx] = physaddr;
					raw_trace[3][raw_trace_idx] = cpu_get_tsc(env) + env->tsc_offset;//DATA_SIZE;
					raw_trace[4][raw_trace_idx++] = env->tlb_table[mmu_idx][index].addr_read+(physaddr&0xfff);
				}else{	
					varify_vaddr += (physaddr&0xfff);
					if(varify_vaddr != trace_pc && !(varify_vaddr >= trace_pc && varify_vaddr <= inst_size)){
						raw_trace[0][raw_trace_idx] = 0x0;
						raw_trace[1][raw_trace_idx] = env->cpuid_apic_id;
						raw_trace[2][raw_trace_idx] = physaddr;
						raw_trace[3][raw_trace_idx] = cpu_get_tsc(env) + env->tsc_offset;//DATA_SIZE;
		...
		if (tlb_addr & ~TARGET_PAGE_MASK) {
			...
			if (memtrace_code){
				...
				if(varify_vaddr!=(trace_pc & 0xfffffffffffff000)){
					...
					raw_trace[2][raw_trace_idx] = physaddr;
					raw_trace[3][raw_trace_idx] = cpu_get_tsc(env) + env->tsc_offset;//DATA_SIZE;
					raw_trace[4][raw_trace_idx++] = env->tlb_table[mmu_idx][index].addr_write+(physaddr&0xfff);
				}else{	
					varify_vaddr += (physaddr&0xfff);
					if(varify_vaddr != trace_pc && !(varify_vaddr >= trace_pc && varify_vaddr <= inst_size)){
						raw_trace[0][raw_trace_idx] = 0x1;
